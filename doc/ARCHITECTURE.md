...
- **Campos Principais (tipados):**
  - `theme: ThemeName` – union de nomes de tema (`'default'`, `'finey-workbench-1'`, `'finey-workbench-2'`, `'finey-workbench-3'`, `'finey-nexus-1'`, `'finey-hub-1'`, `'windows-xp'`, `'windows-7'`, `'christmas'`).
  - `animationsEnabled?: boolean` – liga/desliga animações globalmente.
  - `effectsEnabled?: boolean` – controle global de efeitos.
  - `themeEffectsEnabled?: boolean` – ativa/desativa efeitos de fundo automáticos dos temas (ex: corações no tema Puffy). **Regra de Ativação**: Requer esta flag como `true` e a presença da prop `bgEffect` no componente (ex: `<fy-layout bgEffect>`).
  - `wallpaperEnabled?: boolean` – ativa/desativa o sistema de papéis de parede globais. **Regra de Ativação**: Requer esta flag como `true` e a presença da diretiva `fyWallpaper` no elemento.
  ...
  - `tokenOverrides?: DeepPartial<DesignTokens>` – árvore de tokens fortemente tipada para sobrescrever parcialmente o tema ativo (ex.: apenas `colors.primary`, `layout.header.height`, `effects.card.shadow`, `effects.toast.background`, `effects.table.headerBackground`, etc.).
  - `componentAnimationsOverrides?: ComponentAnimationsOverrides` – mapa tipado de animações por componente/evento que sobrescreve as animações definidas no tema (ex.: `'fy-button'.hover`, `'fy-input'.focus`, `'fy-layout'.enter`, `'fy-slot:sidebar'.open`, `'fy-card'.enter`, `'fy-table'.enter`).
  - `http?: HttpConfig` – configurações globais para o `FyWebClientService` (baseUrl, timeout, retries, etc.).
  - `effectsEnabled?: boolean` – controle global de efeitos.
  - `themeEffectsEnabled?: boolean` – habilita efeitos automáticos de tema.
  - `wallpaperEnabled?: boolean` – habilita o sistema de wallpapers.
  - `disableEffectsForComponents?: ComponentSelector[]` – desativa efeitos por seletor de componente.
  - `effectTriggers?: Partial<Record<UIEventKey, EffectName>>` – mapa tipado de eventos de UI (`'fy-button.click'`, `'fy-input.focus'`, `'fy-layout.enter'`, `'fy-slot:sidebar.open'`, `'fy-slot:sidebar.close'`, `'fy-card.submit'`, `'fy-table.rowClick'`, `'fy-toast.open'`) para nomes de efeitos globais (`'confetti'`, `'hearts'`, `'window-open'`, `'sidebar-slide-in'`, `'sidebar-slide-out'`, `'window-macos-sheet-open'`, `'window-macos-sheet-close'`).
  - `logging: LoggingConfig` – configuração centralizada de logs:
    - `enabled: boolean` – ativa/desativa todos os logs globalmente (interruptor mestre).
    - `level: 'debug' | 'info' | 'warn' | 'error'` – nível mínimo de log.
    - `console: { enabled: boolean }` – controla logs no console do navegador.
    - `localFiles: { enabled: boolean, path: string, filenamePattern?: string }` – controla os logs persistidos em arquivos locais (via servidor de apoio). O fyLib detecta se o `path` é relativo ou absoluto.
    - `remote: { enabled: boolean, endpoint: string }` – controla o envio de logs para um servidor de telemetria externa.
...
- **Tokens Estruturais, de Efeito e Wallpapers:** Além de cores, espaçamentos e tipografia, o tema expõe tokens de layout como `layout.header.height`, `layout.header.background`, `layout.sidebar.width`, `layout.sidebar.background`, `layout.sidebar.shadow`, `layout.content.padding`, e também tokens de efeitos como `effects.button`, `effects.window`, `effects.input`, `effects.card`, `effects.toast`, `effects.table`, `effects.chart`, que são convertidos em variáveis CSS.
  - Temas podem definir um `backgroundEffect` (ex: `hearts`) e um `wallpaper` (padrão SVG animado) para criar experiências imersivas automáticas.
  ...
  - `effects.table` para `background`, `borderColor`, `headerBackground`, `rowHoverBackground`, `stripedBackground`, `textColor`, `headerTextColor`.
  - `effects.chart` para `background`, `gridColor`, `labelColor`, `colors` (paleta de séries).
  - `effects.toast` para `background`, `borderColor`, `textColor`, `shadow`, `borderRadius`, `padding`, `gap`, `iconSize`, `iconColor` (por tipo), `icons` (mapeamento de nomes).
...
### 7️⃣ @fylib/testing (Em progresso)
A infraestrutura de testes do fyLib utiliza **Vitest** para os pacotes do monorepo e ferramentas nativas dos adapters (ex.: Angular Testing Library).
- **Unit Tests**: Focados em lógica de negócio nos pacotes core.
- **Component Tests**: Verificam renderização e acessibilidade nos adapters.
- **Acessibilidade (A11y)**: O fyLib segue os padrões WCAG 2.1 AA, garantindo atributos ARIA corretos e navegação por teclado em todos os componentes.
